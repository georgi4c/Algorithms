        ...
        public static Item[] FillKnapsack(Item[] items, int capacity)
        {
            ...
            for (int i = 1; i < items.Length; i++)
            {
                for (int c = 0; c <= capacity; c++)
                {
                    ...
                    {
                        var sumWithCurrent = items[i].Price + maxPrice[i - 1, remainingCapacity];
                        if (sumWithCurrent >= maxPrice[i, c])
                        {
                            maxPrice[i, c] = sumWithCurrent;
            ...
            while (itemIndex >= 0)
            {
                var checker = isItemTaken[itemIndex, capacity];
                if (checker)
                {
