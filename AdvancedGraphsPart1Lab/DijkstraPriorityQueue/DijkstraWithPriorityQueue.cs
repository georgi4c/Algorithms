    ...
    public static class DijkstraWithPriorityQueue
    {
        public static List<int> DijkstraAlgorithm(Dictionary<Node, Dictionary<Node, int>> graph, Node sourceNode, Node destinationNode)
        {
            ...
            {
                pair.Key.DistanceFromStart = double.MaxValue;
            }
            ...
            while (priorityQueue.Count > 0)
            {
                ...
                foreach (var edge in graph[currentNode])
                {
                    ...
                    var distance = edge.Value + currentNode.DistanceFromStart;
            ...
            if (Math.Abs(destinationNode.DistanceFromStart - double.MaxValue) < 1)
            {
                return null;
            }
